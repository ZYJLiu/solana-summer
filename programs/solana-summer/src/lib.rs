...
pub mod solana_summer {
    ...
    pub fn create_reward_mint(
        ...
        token_data.rebate_basis_points = rebate_basis_points;
        token_data.bonus_basis_points = bonus_basis_points;
...
pub struct CreateTokenReward<'info> {
    #[account(
        init,
        seeds = ["RewardData".as_bytes().as_ref(), user.key().as_ref()],
        bump,
        payer = user,
        space = 8 + 32 + 32 + 1 + 8 + 8
    )]
...
pub struct TokenData {
    ...
    pub rebate_basis_points: u64, // 8
    pub bonus_basis_points: u64,  // 8
}
